...
DESCRIPTION
	...
	To enter a negative number, begin the number with ‘‘_’’. ‘‘-’’ cannot be used for this, as it is a binary operator
	for subtraction instead. To enter two numbers in succession, separate them with spaces or newlines.
	These have no meaning as commands.

*)

(* 2) The ideal representation is to use eight-digit numbers in a
list. store only one digit in each element of a list. 
represent your numbers with the lowest order digit at the front of the list and
the leftmost digit at the end. *)
...
(* -INPUT and OUTPUT-------------------------------------------------------------------------------- *)
(* ) First implement input and output of numbers. Make sure your output duplicates
dc for very large numbers. Note that an underscore prefixing a number
makes it negative. The minus sign is strictly for subtraction. *)
...
(* ----------------------------------------------------------------------------------------------- *)
    ...
    let rec string_of_bigint' value position =
        if (position = 0) then (string_of_int (car value):: (string_of_bigint' (cdr value) (position+1))) else (
...
(* ) Recursive functions called on by implimented functions that do the brute work for the function called *)
...
let rec add' list1 list2 carry = 
	...
	(* ) NOTES:
		...
		comparable = bignum - all the levels that were able to fit insdie new comparable after first
...
(* ) x to the power of n *) 
    ...
    let pow (Bigint (sign1, x)) (Bigint (sign2, n)) = 
    	if sign2 = Pos then (
	    	if sign1 = Pos then Bigint (Pos, goodbyezeros (pow' x n)) (* if x is Pos, return Pos *)
	    	else if (even x) then Bigint (Pos, goodbyezeros (pow' x n)) (* if x is Neg, but powered to a pos num, return Pos *)
	   		else Bigint (Neg, goodbyezeros (pow' x n)) (* if x is Neg, but powered to a neg num, return Neg *)
	   	)
	   	else zero
